...
{{/* railmcp-orchestrator-internal-token key resolution -- the shared
secret between railMcp's own /token orchestrator-session grant and
orchestrator/'s token-acquisition step (embedded-chatbot-option-b plan,
Task 1/3/6). Lives under railMcp's own existingSecret/key resolution
(mirrors railMcpInternalCompleteTokenSecretKey exactly) since railMcp is
the one that validates it; orchestrator-deployment.yaml reuses this same
helper for its own identically-named env var, the same cross-reference
frontend-deployment.yaml already makes for RAILMCP_INTERNAL_COMPLETE_TOKEN. */}}
{{- define "distant-signal.railMcpOrchestratorInternalTokenSecretKey" -}}
{{- if .Values.railMcp.existingSecret }}
{{- .Values.railMcp.existingSecretOrchestratorInternalTokenKey }}
{{- else }}
{{- print "orchestrator-internal-token" }}
{{- end }}
{{- end }}

{{/*
orchestrator: the DS-hosted chat orchestrator (embedded-chatbot-option-b
plan, Task 3/6) -- built and published by THIS repo's own CI, unlike
railMcp above. Object name, in-cluster URL, and Secret name/key
resolution, same shape as the railMcp helpers immediately above.
*/}}
{{- define "distant-signal.orchestratorFullname" -}}
{{- printf "%s-orchestrator" (include "distant-signal.fullname" .) | trunc 63 | trimSuffix "-" -}}
{{- end }}

{{- define "distant-signal.orchestratorSecretName" -}}
{{- default (include "distant-signal.secretName" .) .Values.orchestrator.existingSecret }}
{{- end }}

{{- define "distant-signal.orchestratorAnthropicApiKeySecretKey" -}}
{{- if .Values.orchestrator.existingSecret }}
{{- .Values.orchestrator.existingSecretAnthropicApiKeyKey }}
{{- else }}
{{- print "anthropic-api-key" }}
{{- end }}
{{- end }}

{{/* In-cluster URL for the orchestrator Service -- consumed by
frontend-deployment.yaml's ORCHESTRATOR_BASE_URL, mirroring
distant-signal.railMcpInClusterUrl's own shape exactly. Internal cluster
DNS, deliberately not a public URL: this call never leaves the cluster
(Decision 2). */}}
{{- define "distant-signal.orchestratorInClusterUrl" -}}
{{- printf "http://%s:%d" (include "distant-signal.orchestratorFullname" .) (int .Values.orchestrator.service.port) -}}
{{- end }}
