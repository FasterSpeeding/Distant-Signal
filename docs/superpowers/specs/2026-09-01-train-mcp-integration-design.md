...
6. **Individual-train (TRUST) corroboration, only when a real match
   exists — not a general mechanism.** If (and only if) the leg's `(uid,
   date)` corresponds to a train the *calling identity* has separately,
   already pinned in DS via `POST /Train/track`, `GET
   ...
   cookie** (`AuthenticatedUser`, `train.rs:221`) — under Decision 4's
   recommended auth posture (Option 1: the derived service calls DS
   anonymously), the derived service has no DS session at all, so this
   tier is not reachable by design in the near term. It becomes reachable
   only if a future iteration adopts shared identity (Decision 4, Option 2)
   *and* the specific human happens to have independently tracked that
   exact train in DS themselves — a real but narrow, opportunistic case,
   not something this design engineers a path for. Stated plainly rather
   than glossed over, since the task specifically asked this scoped
   correctly: **this is not a general per-leg mechanism, it only ever
   applies to the subset of legs that happen to already be a DS-tracked
   train for the same identity as the caller.**
...
### 4. Auth: anonymous HTTP client to DS's public routes — no session, no shared identity, for this pass

Per the research doc's own three sketched options, **Option 1 (keep
train-mcp's existing Discord OAuth for gating who can use the MCP tools at
all; call DS's public API as a plain anonymous client)** is chosen for
this design, confirmed viable by this session's re-verification that
every route this spec depends on — `/public/stations`, `/Line/.../Status`,
`/public/lines`, `/public/lines/{id}/definition`, `/public/incidents/{id}`
— requires no session and no internal token (Current relevant state,
above).

- **Option 2 (reuse DS's OIDC SSO as the derived service's own auth)**:
  rejected for this pass, not permanently. It's the option that would
  make the Tier-1 TRUST-corroboration path (3b.6) generally reachable,
  but the research doc's own finding stands unrevised: it's a real scope
  increase (MCP clients commonly assume Dynamic Client Registration, which
  Discord doesn't support and DS's own OIDC provider's support for was not
  checked either pass) unjustified by what this spec actually needs —
  every capability designed here (`resolve_station`, DS-sourced line
  status/incident annotation) works fully under Option 1.
- **Option 3 (no auth at all)**: this is, in effect, what Option 1
  already reduces to for every DS call this spec makes — DS's own routes
  are unauthenticated regardless of what gates access to the MCP tools
  themselves. The distinction the research doc drew (Option 3 = no gate
  on the MCP tools either) is **not** adopted: train-mcp's existing
  Discord allowlist stays as the gate on who can call the MCP server at
  all, unchanged — this spec only concerns itself with how the derived
  service authenticates to DS, not who's allowed to use the derived
  service.
...
New Helm subchart template, `charts/distant-signal/templates/railmcp-*.yaml`,
mirroring `schedulefeed-*.yaml`'s single-external-image shape (simpler
than `schedulefeed`'s multi-container pod, since this service has no
paired verifier container):

- `railmcp-deployment.yaml`: one container, image
  `{repository, tag}` from `values.yaml` under a new `railMcp:` block,
  referenced by **full external registry path** (e.g.
  `ghcr.io/<org>/distant-signal-mcp`), not a bare `distant-signal/<name>`
  short name — per Decision 1, this repo's own CI does not build this
  image, matching `devauthentik-server-deployment.yaml`'s precedent for
  `ghcr.io/goauthentik/server`. `env`: `DS_API_BASE_URL` (analogous to
  `frontend-deployment.yaml`'s existing `API_BASE_URL`, pointing at the
  in-cluster `api` service DNS name — no new DS-side change needed to
  support this), plus whatever Discord bot/OAuth secrets the fork's own
  `.env` shape already expects (mounted the same way
  `schedulefeed-secret.yaml` mounts SFTP host-key material — a
  Kubernetes `Secret`, not baked into the image).
- `railmcp-service.yaml`: a `ClusterIP` exposing the MCP transport's port,
  matching `schedulefeed-service.yaml`'s shape for a single-container
  service (no external ingress designed here — out of scope, see below).
- `values.yaml`: a `railMcp:` block gated by `railMcp.enabled` (default
  `false`, same pattern as `.Values.scheduleFeed.enabled`), so this
  remains an opt-in deployment for anyone running this chart who doesn't
...
hostname every other compose service already uses to reach `api`),
image referenced the same way `schedule-sftp`'s entry references its
third-party image (not a `build:` context pointing into this repo, since
the source doesn't live here). `docker-compose.dev.yml`: left to a later
implementation pass to decide whether local dev wants a bind-mount of a
sibling checkout of the derived service's own repo (for live-reloading
during development) — sketched as a real option, not designed further
here, matching this spec's overall depth for deployment (this repo's other
integration specs don't design network policy or scaling either, and this
one doesn't need to).
...
│  │  3. GET /Line/{id}/Status?detail=true (DS, anonymous)           │  │
│  │  4. scope check: disruption.affectedStops/affectedRoutes        │  │
│  │     vs. leg's own origin/destination CRS                        │  │
│  │  5. (narrow case) GET /Train/by-uid/{uid}/{date} — only if      │  │
│  │     caller has a DS session AND already tracks that train       │  │
│  └─────────────────────────────────────────────────────────────────┘  │
└───────────────────────────┬──────────────────────────────────────────┘
                             │ plain HTTP, anonymous
                             ▼
                  Distant Signal `api` (crates/api)
                  /public/*, /Line/*/Status  (unauthenticated)
                  /Train/by-uid/*  (requires DS session — rarely reachable
                                     here, see 3b.6)
```
...
- **Auth (Decision 4)**: a smoke test confirming every DS call this
  service makes carries no `Authorization`/session cookie header —
  guards against an accidental future dependency on DS session state that
  would silently break once removed, given Decision 4's explicit choice
...
- **Reusing DS's OIDC SSO as the derived service's own auth (research
  doc's Option 2).** Rejected for this pass in Decision 4; would need its
  own design (including checking DS's OIDC provider's Dynamic Client
  Registration support) if ever pursued.
- **NRE/Network-Rail-branding attribution requirements for MCP
  ...
  — this spec does not resolve it, only flags it.
- **Ingress/TLS/network-policy/autoscaling for the new `railMcp` chart
  component.** Sketched at the same shallow depth this repo's other
  integration specs give deployment (Decision 6) — not designed further,
...
3. **The TRUST-corroboration tier (3b.6) is reachable only in a narrow,
   opportunistic case under the recommended Auth Option 1** (Decision 4)
   — a caller would need to independently hold a DS session and have
   separately tracked the exact same train. This is stated plainly as a
   near-term limitation, not a gap this design tries to close; closing it
   would require Decision 4's Option 2 (shared OIDC identity), which
   carries its own real scope (Dynamic Client Registration support,
   unchecked against DS's OIDC provider).
4. **The NRE-attribution "presentation" question (Licensing note) is
   unresolved** and should be checked before this ships, not assumed
   either way.
