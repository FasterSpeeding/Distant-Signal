...
- **Account credentials for DTD's push client to authenticate against this
  ...
  primary-source re-check," above).

  **CORRECTION, 2026-09-01 (repo owner):** this document originally said
  the *password* half of this credential was "never auto-generated by the
  chart — a random password... would be meaningless without DTD's side
  registering it," reasoning by analogy to `pollers.*.apiKey`. That analogy
  doesn't hold and was wrong: `pollers.*.apiKey` authenticates *this app*
  against an *external* system that already assigned the key, so a random
  guess really would be meaningless there — but here it is *this app's own
  server* minting the account DTD authenticates against, exactly the same
  shape as the host key immediately above (this app decides the value,
  then tells DTD what it is, not the other way around). The password now
  IS auto-generated by the chart (and preserved across `helm upgrade`s,
  same lookup-preserve reasoning as the host key), with a fixed default
  username (`dtd-push`) alongside it — see
  `charts/distant-signal/templates/schedulefeed-secret.yaml` and
  `values.yaml`'s `scheduleFeed.sftp.username`/`password`. The public-key
  half of this bullet is unaffected by this correction and remains
  correct as originally written: a public key for a server authenticating
  a client is *the client's* (DTD's) artifact to supply — generating one
  with no real matching DTD private key behind it would be the genuinely
  meaningless case the original reasoning described, just not the one
  that actually applies to the password.
...
**Superseded by the real `docker-compose.yml`** (and `local.env.example`/
`dev.env.example`), which has since diverged from this sketch in several
ways as the implementation plan progressed — notably the credential
correction above (`SCHEDULE_SFTP_USERNAME` now defaults to `dtd-push`
rather than being a required-with-no-default placeholder; the sketch below
is left as historical context, not updated to match).
