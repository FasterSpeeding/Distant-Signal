...
| Decision 6: accessibility — `role="img"`/`aria-label` (**superseded 2026-09-15: `role="group"`** — see Global Constraints and the design doc's Decision 6 correction), `aria-hidden` nodes except focusable Tooltip triggers, color-never-only-signal, `prefers-reduced-motion` | Task 1 (`role="img"` base); Task 5 (full `aria-label` text); Task 3/6 (aria-hidden circles + focusable Tooltip trigger, tested explicitly in Task 6); Task 2 (halo is a shape/shadow difference, not a color-only signal); Task 4 (reduced-motion) |
