...
- **CORRECTED 2026-09-01 (repo owner) — supersedes this bullet's original
  text below.** The password half of the account credential DOES now
  auto-generate (fixed default username `dtd-push`, auto-generated and
  upgrade-preserved password) — see the push design doc's own correction
  note in its Credentials section and `charts/distant-signal/templates/
  schedulefeed-secret.yaml`'s corrected comment for the full reasoning
  (this app mints the account, unlike `pollers.*.apiKey`, so there's
  nothing external for a random value to fail to match). The public-key
  half and `authMethod`'s no-default rule are unaffected. Original text,
  left for history rather than silently rewritten:
