...
describe('RenameTicketButton', () => {
  ...
  it('POSTs the trimmed name and refreshes on success', async () => {
    ...
    fireEvent.change(input, { target: { value: "  Mum's ticket to Leeds  " } });
    fireEvent.click(screen.getByRole('button', { name: 'Save' }));

    await waitFor(() => {
      expect(fetchMock).toHaveBeenCalledWith('/api/Train/tickets/7/name', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ customName: "Mum's ticket to Leeds" }),
      });
    });
  ...
  it('Clear is only shown when a custom name is currently set, and posts null', async () => {
    ...
    renderWithMantine(
      <RenameTicketButton ticketId={7} customName="Mum's ticket to Leeds" defaultName="LNER — Off-Peak Day Single" />,
    );
    fireEvent.click(screen.getByRole('button', { name: 'Rename' }));
  ...
  it('a 401 shows a login prompt instead of the raw backend error text', async () => {
    ...
    await screen.findByRole('link', { name: 'Log in to rename this ticket' });
