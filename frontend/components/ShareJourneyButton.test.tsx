...
// `useGroupSummaries` reads from `GroupSummariesProvider`'s context instead
// of fetching `/api/groups` itself -- every render below supplies its own
// groups value directly, synchronously, rather than mocking that fetch (see
// `lib/useGroupSummaries.tsx`'s own doc comment for why: the value is
// present at first render, no fetch-then-setState chain to await).
...
describe('ShareJourneyButton', () => {
  ...
  it('renders nothing when the viewer is in zero groups', () => {
    renderWithGroups(<ShareJourneyButton journeyId={7} />, []);
  ...
  it('renders nothing for an anonymous visitor (null groups)', () => {
    renderWithGroups(<ShareJourneyButton journeyId={7} />, null);
    expect(screen.queryByRole('button')).not.toBeInTheDocument();
  });
  ...
  it('renders nothing with no GroupSummariesProvider in the tree at all', () => {
    renderWithMantine(<ShareJourneyButton journeyId={7} />);
    expect(screen.queryByRole('button')).not.toBeInTheDocument();
  });
  ...
  it('shows the button and a group picker once the viewer has at least one group', async () => {
    ...
    const button = await screen.findByRole('button', { name: 'Share with a group' });
  ...
  it('POSTs the chosen groupId and shows a success confirmation without closing the modal', async () => {
    ...
    await waitFor(() => {
      expect(fetchMock).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ method: 'POST', body: JSON.stringify({ journeyId: 7 }) }),
      );
    });
  ...
  it('shows a login prompt on a 401 rather than the raw rejection text', async () => {
    ...
    expect(await screen.findByText('Log in to share this journey')).toBeInTheDocument();
