...
# Multi-stage build for the `schedule-ingest` service.
#
# Builder pin: matches poller-incidents/poller-stations/poller-tfl/poller-tocs/
# poller-ldbws at rust:1.88-bookworm — this crate pulls in the same reqwest ->
# url -> idna -> idna_adapter -> icu_normalizer/icu_provider transitive chain
# (through the `common` crate every service in this workspace depends on)
# requiring rustc 1.88+ -- confirmed by actually building poller-ldbws's image
# against rust:1.86-bookworm first and hitting:
...
#   docker build -f docker/schedule-ingest.Dockerfile .
#
...
# RUN — which is why the runtime stage below copies from /usr/local/bin/schedule-ingest.
RUN --mount=type=cache,id=cargo-registry,target=/usr/local/cargo/registry,sharing=locked \
    --mount=type=cache,id=cargo-git,target=/usr/local/cargo/git,sharing=locked \
    --mount=type=cache,id=cargo-target-1.88,target=/app/target,sharing=locked \
    if [ "$CARGO_PROFILE" = "release" ]; then \
      cargo build --release --bin schedule-ingest; \
    else \
      cargo build --bin schedule-ingest; \
    fi \
    && cp /app/target/${CARGO_PROFILE}/schedule-ingest /usr/local/bin/schedule-ingest
