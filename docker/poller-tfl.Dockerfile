...
# Multi-stage build for the `poller-tfl` service.
#
...
#   docker build -f docker/poller-tfl.Dockerfile .
#
...
# RUN — which is why the runtime stage below copies from /usr/local/bin/poller-tfl.
RUN --mount=type=cache,id=cargo-registry,target=/usr/local/cargo/registry,sharing=locked \
    --mount=type=cache,id=cargo-git,target=/usr/local/cargo/git,sharing=locked \
    --mount=type=cache,id=cargo-target-1.88,target=/app/target,sharing=locked \
    if [ "$CARGO_PROFILE" = "release" ]; then \
      cargo build --release --bin poller-tfl; \
    else \
      cargo build --bin poller-tfl; \
    fi \
    && cp /app/target/${CARGO_PROFILE}/poller-tfl /usr/local/bin/poller-tfl
...
COPY --from=builder /usr/local/bin/poller-tfl /usr/local/bin/poller-tfl

USER poller

ENTRYPOINT ["/usr/local/bin/poller-tfl"]
