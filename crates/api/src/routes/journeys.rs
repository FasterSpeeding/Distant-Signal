...
async fn post_journey(
    ...
    // Low finding #2 (2026-09-25 review): every OTHER `custom_name` write
    // path in this codebase (tracked-train rename, ticket rename, group
    // rename) runs the caller's string through
    // `train_tracking::validate_custom_name` first, capping it at
    // `common::CUSTOM_NAME_MAX_LENGTH`. This route used to skip that
    // entirely and hand `body.custom_name` straight to
    // `create_journey_with_*_leg`, letting a multi-KB name get stored and
    // rendered to every group member this journey is ever shared with.
