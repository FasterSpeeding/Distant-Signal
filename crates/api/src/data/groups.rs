...
struct GroupCustomLineRow {
    ...
    granted_by_email: Option<String>,
}
...
impl From<GroupCustomLineRow> for GroupCustomLine {
    fn from(row: GroupCustomLineRow) -> Self {
        GroupCustomLine {
            ...
            // Same "name, else email, else nothing" collapse
            // `GroupTrain::from` applies -- never the raw internal user id.
            granted_by_name: row.granted_by_name.or(row.granted_by_email),
        }
    }
}
...
pub async fn list_group_custom_lines(
    ...
    let rows: Vec<GroupCustomLineRow> = sqlx::query_as(
        "SELECT g.line_id, cl.name AS line_name, \
                g.granted_by, u.name AS granted_by_name, u.email AS granted_by_email \
...
pub async fn list_shared_custom_lines_for_user(
    ...
    let rows: Vec<SharedCustomLineRow> = sqlx::query_as(
        "SELECT g.id AS group_id, g.name AS group_name, \
                ...
                gr.granted_by, u.name AS granted_by_name, u.email AS granted_by_email \
         FROM group_members me \
         JOIN groups g ON g.id = me.group_id \
...
mod custom_line_grant_wire_shape_tests {
    ...
    /// The same `name`-else-`email`-else-nothing collapse every other
    /// attribution field in this module applies.
    #[test]
    fn granted_by_name_falls_back_to_email_then_to_nothing() {
        let row = |name: Option<&str>, email: Option<&str>| GroupCustomLineRow {
            line_id: "custom-x".to_string(),
            line_name: "X".to_string(),
            granted_by: "user-1".to_string(),
            granted_by_name: name.map(str::to_string),
            granted_by_email: email.map(str::to_string),
        };
        assert_eq!(
            GroupCustomLine::from(row(Some("Alex"), Some("alex@example.com"))).granted_by_name,
            Some("Alex".to_string())
        );
        assert_eq!(
            GroupCustomLine::from(row(None, Some("alex@example.com"))).granted_by_name,
