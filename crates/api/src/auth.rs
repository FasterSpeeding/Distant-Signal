...
pub async fn require_internal_oauth(
    ...
    // path only.
    let path_matches: Vec<_> = app.internal_oauth_routes.iter().filter(|(prefix, _, _)| path.starts_with(prefix)).collect();
    ...
    let Some((_, _, required_groups)) = path_matches.into_iter().find(|(_, entry_method, _)| *entry_method == method)
    else {
...
mod route_scoping_tests {
    ...
    fn test_router(app: App) -> axum::Router {
        ...
        crate::app::Router::new()
            .fallback(ok)
            .layer(middleware::from_fn_with_state(app.clone(), require_internal_oauth))
            .with_state(app)
    }
    ...
    async fn send(router: &axum::Router, method: Method, path: &str, token: Option<&str>) -> StatusCode {
        let mut builder = Request::builder().method(method).uri(path);
        if let Some(token) = token {
            builder = builder.header(header::AUTHORIZATION, format!("Bearer {token}"));
        }
        let request = builder.body(Body::empty()).expect("build request");
        router.clone().oneshot(request).await.expect("oneshot request").status()
    }
    ...
    async fn trust_consumers_token_is_accepted_on_get_stanox_crs() {
        ...
        let token = token_for(&server.uri(), "svc-trust-consumer-1", &["svc-trust-consumer"]);

        assert_eq!(send(&router, Method::GET, "/stanox-crs", Some(&token)).await, StatusCode::OK);
    }
    ...
    async fn trust_consumers_token_is_rejected_on_post_stanox_crs() {
        ...
        assert_eq!(send(&router, Method::POST, "/stanox-crs", Some(&token)).await, StatusCode::FORBIDDEN);
    }
    ...
    async fn schedule_references_token_is_accepted_on_post_stanox_crs() {
        let (server, app, _routes) = test_app().await;
        let router = test_router(app.clone());
        let token = token_for(&server.uri(), "svc-schedule-reference-1", &["svc-schedule-reference"]);

        assert_eq!(send(&router, Method::POST, "/stanox-crs", Some(&token)).await, StatusCode::OK);
    }
    ...
    async fn schedule_references_token_is_rejected_on_get_stanox_crs() {
        ...
        assert_eq!(send(&router, Method::GET, "/stanox-crs", Some(&token)).await, StatusCode::FORBIDDEN);
    }
    ...
    async fn a_token_with_no_matching_group_at_all_is_rejected_on_stanox_crs() {
        let (server, app, _routes) = test_app().await;
        let router = test_router(app.clone());
        let token = token_for(&server.uri(), "svc-poller-incidents-1", &["svc-poller-incidents"]);
    ...
    async fn a_request_with_no_bearer_token_is_rejected_unauthorized() {
        ...
        assert_eq!(send(&router, Method::GET, "/stanox-crs", None).await, StatusCode::UNAUTHORIZED);
    }
    ...
    async fn a_path_absent_from_the_table_entirely_is_rejected_forbidden_even_with_a_valid_token() {
        ...
        assert_eq!(send(&router, Method::GET, "/some-unknown-route", Some(&token)).await, StatusCode::FORBIDDEN);
    }
    ...
    async fn every_production_route_entry_is_reachable_by_its_own_caller_on_its_own_method() {
        ...
        for (path, method, groups) in &routes {
            assert_eq!(groups.len(), 1, "expected exactly one group per route entry post-fix: {path} {method}");
            let token = token_for(&server.uri(), "svc-under-test", &[groups[0].as_str()]);

            let status = send(&router, method.clone(), path, Some(&token)).await;
            assert_eq!(status, StatusCode::OK, "expected {method} {path} to accept its own caller's token");
        }
    }
}
